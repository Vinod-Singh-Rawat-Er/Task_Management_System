...
@section('content')
        ...
        <div class="row">
            ...
            <div class="card">
                ...
                <table id="example2" class="table table-bordered table-hover">
                    ...
                    <tbody>
                    @foreach( $owners as $crse)
                        <tr>
                        <td>{{ $crse->name }}</td>
                        <td>{{ $crse->father_name }}</td>
                        <td>{{ $crse->phone_no }}</td>
                        <td>{{ $crse->email }}</td>
                        <td>{{ $crse->distt }}</td>
                        <td>{{ $crse->address }}</td>
    ...
    <div class="bs-canvas bs-canvas-right position-fixed bg-light h-100 {{ old() ? ' mr-0': '' }}">
        <header class="bs-canvas-header p-3 bg-dark overflow-auto">
            ...
            <h4 class="d-inline-block text-light mb-0 float-right"><span class="course_action">{{old('id')!= '' ? 'Update' : 'Add'}}</span> Owner</h4>
        </header>
        <div class="bs-canvas-content px-3 py-1">
           

                <form action="{{ route('owner.store') }}" enctype="multipart/form-data" method="post">
                    @csrf
                    @if(old('id')!='')
                    @method('put')
                    @else
                    ...
                    <div class="form-group">
                        <label for="exampleInputName">Father Name / husband name</label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="phone_no" class="form-control isExist" id="phone_no" value="{{ old('phone_no')}}" 
                            placeholder="" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="email" class="form-control isExist" id="email" value="{{ old('email')}}"
                            placeholder="" required>
                        <span style="color:red">@error('email'){{ $message }}@enderror</span>
                    </div>
                    <div class="form-group">
                        <label for="id_card_type">Id Card</label>
        
                        <select name="id_card_type" class="form-control" id="id_card_type" required>
                            <opton value="">Select Id-Card</option>
                            <option value="Voter Id" {{ old('id_card_type') == 'Voter Id' ? 'Selected':'' }}>Voter Id</option>
                            <option value="Aadhar Card" {{ old('id_card_type') == 'Aadhar Card' ? 'Selected':'' }}>Aadhar Card</option>
                            <option value="Passport Id" {{ old('id_card_type') == 'Passport Id' ? 'Selected':'' }}>Passport Id</option>
                            <option value="Driving Licence" {{ old('id_card_type') == 'Driving Licence' ? 'Selected':'' }}>Driving Licence</option>
                        </select>
                        <span style="color:red">@error('email'){{ $message }}@enderror</span>
                    </div>
                    <div class="form-group">
                        <label for="address">Id Card Number</label>
        
                        <input type="text" name="id_card_number" class="form-control isExist" id="id_card_number" value="{{ old('id_card_number')}}"
                            placeholder="" required>
                        <span style="color:red">@error('id_card_number'){{ $message }}@enderror</span>
                    </div>
                    <div class="form-group">
...
<script>
    ...
    function editCourse(id, ref){
        ...
        $.ajax({
            ...
            dataType: "JSON",
            success: function (response) {
                $.each(response.owner, function (indexInArray, valueOfElement) {
